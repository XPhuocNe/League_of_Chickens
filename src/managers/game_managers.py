...
class MenuManager:
    ...
    def draw_main_menu(self):
        ...
        # Hướng dẫn
        title = self._font_sub.render("── ĐIỀU KHIỂN ──", True, COL_SCORE)
        self.screen.blit(title, (cx - title.get_width() // 2, 420))
        ...
        for i, (label, desc) in enumerate(rows):
            y = 446 + i * 26
            t_label = self._font_small.render(label, True, COL_TEXT)
            t_colon = self._font_small.render(":", True, COL_TEXT)
            t_desc  = self._font_small.render(desc,  True, COL_TEXT)
            self.screen.blit(t_label, (col_label, y))
            self.screen.blit(t_colon, (col_colon, y))
            self.screen.blit(t_desc,  (col_desc,  y))
...
class GameManager:
    ...
    def update(self, dt: float):
        ...
        # Collision: player ← laser beam của boss
        for b in self.bosses:
            for laser in b.lasers:
                if laser.active and laser.hits(player.x, player.y, player.radius):
                    player.take_damage(laser.dmg)
        for p in self.projectiles:
