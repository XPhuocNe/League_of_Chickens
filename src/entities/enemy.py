...
class Enemy:
    ...
    def __init__(self, x: float, y: float, etype: int = 1, map_type: str = "fire"):
        ...
        self.hit_flash = 0.0
        self._dying    = False   # True khi hp=0, chờ hit_flash chạy hết rồi mới alive=False
        self.color     = COL_ENEMY1 if etype == 1 else COL_ENEMY2

        self._facing          = 1
        self._attack_angle    = 90.0   # góc sprite hướng xuống ban đầu (độ, pygame)
        self._display_angle   = 90.0   # góc hiển thị thực tế — lerp chậm khi idle
    ...
    def update(self, dt: float, player, projectiles: list,
        ...
        # Góc target tới player
        target_angle = math.degrees(
            math.atan2(player.y - self.y, player.x - self.x)
        )
        self._attack_angle = target_angle

        # Luôn lerp chậm 80°/giây — snap sẽ xảy ra trong _fire khi thực sự bắn
        TURN_SPEED = 80.0
    ...
    def _fire(self, player, projectiles: list, delayed_attacks: list, tracker=None):
        ...
        if self.etype == 1:
            ...
            # Snap góc ngay lúc khai hỏa
            self._display_angle = math.degrees(math.atan2(dy2, dx2))
            ...
            proj = Projectile(
                ...
                sprite_name="water_arrow" if self._side == "blue" else "fire_arrow",
            )
            ...
            delayed_attacks.append(DelayedCircleAttack(
                ...
                explosion_type="water_explosion" if self._side == "blue" else "fire_explosion",
            ))
    ...
    def draw(self, surface: pygame.Surface, camera):
        ...
        if frame:
            # Sprite gốc quay mặt xuống dưới (90°), rotate về hướng display
            # Khi idle: lerp chậm; khi tấn công: snap ngay
            angle = 90.0 - self._display_angle
            frame = pygame.transform.rotate(frame, angle)
            if self.hit_flash > 0:
                frame = frame.copy()
                alpha_val = int(180 * (self.hit_flash / 0.12))
                # Tạo white layer có cùng alpha shape với frame:
                # blit frame lên white surface với BLEND_RGBA_MULT
                # → white_layer chỉ còn alpha ở đúng chỗ sprite có nội dung
                white = pygame.Surface(frame.get_size(), pygame.SRCALPHA)
                white.fill((255, 255, 255, alpha_val))
                white.blit(frame, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
...
def generate_enemy_cluster(world_y: float, score: int, map_type: str = "fire") -> list:
    ...
    n_type1    = random.randint(2, 2 + int(difficulty * 0.7))
    n_type2    = random.randint(0, 1 + int(difficulty * 0.4))
